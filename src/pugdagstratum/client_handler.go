...
func (c *clientListener) NewBlockAvailable(kapi *PugdagApi) {
	c.clientLock.Lock()
		...
		go func(client *gostratum.StratumContext) {
			...
			if client.WalletAddr == "" {
				if time.Since(state.connectTime) > time.Second*20 { // timeout passed
					// this happens pretty frequepugy in gcp/aws land since script-kiddies scrape ports
			...
			if err != nil {
				if strings.Contains(err.Error(), "Could not decode address") {
					...
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from pugdag, malformed address: %s", err))
					client.Disconnect() // unrecoverable
				} else {
					RecordWorkerError(client.WalletAddr, ErrFailedBlockFetch)
					client.Logger.Error(fmt.Sprintf("failed fetching new block template from pugdag: %s", err))
				}
				return
			}
	...
	if time.Since(c.lastBalanceCheck) > balanceDelay {
		...
		if len(addresses) > 0 {
			go func() {
				balances, err := kapi.pugdagd.GetBalancesByAddresses(addresses)
				if err != nil {
					c.logger.Warn("failed to get balances from pugdag, prom stats will be out of date", zap.Error(err))
					return
				}
