...
var walletRegex = regexp.MustCompile("pugdag:[a-z0-9]+")
...
func CleanWallet(in string) (string, error) {
	...
	if !strings.HasPrefix(in, "pugdag:") {
		return CleanWallet("pugdag:" + in)
	}

	// has pugdag: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:67], nil
	}
	return "", errors.New("unable to coerce wallet to valid pugdag address")
}
